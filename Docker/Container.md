[[Instalação|Anterior: Instalação]]
[[Image|Próximo: Image]] 

Um container é um ambiente de execução isolado que é responsável por executar algum serviço. É criado a partir de uma [image](Estudos/Docker/Image), que define quais serão os arquivos utilizados e comandos que serão executados. A image é um molde para o container. É possível criar vários containers diferentes (embora idênticos) de uma image só. 
...
A partir de agora, iremos utilizar a CLI do docker para estudar o gerenciamento de containeres de forma prática. Entre em algum terminal, após ter [[Instalação|instalado a Docker Engine]], e rode o seguinte comando:
...
* `docker run <args> <image-name>`: cria e roda um container. Recebe vários tipos de argumentos, sendo que dá pra combinar os de uma letra só, desde que só o último possa receber argumentos. (Ex: `-it`: nenhum recebe argumento, `-dp`: o p recebe argumentos e está em último);
	...
	* `-e <KEY>=<DATA>`: cria uma variável de ambiente;
	...
	- `--mount <volume-data>`: seta um volume pro container ([mais](Estudos/Docker/Volume));
	...
	- `--network <network-data>`: seta a network do container ([mais](Estudos/Docker/Network));
